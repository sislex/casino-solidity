...
export class GameCommonService {
  private endTimes = new Map<number, Record<string, number>>();
  ...
  async getGameData(gameId: number): Promise<IGameData> {
    ...
    if (gameDataById.contractAddress) {
      playerData = await this.blockchainService.getGameData(
    ...
    const players = await Promise.all(
      gameDataById.players.map(async (player: GamePlayerDto) => {
        const blockchainPlayer = playerData.players.find(
          (playerBlock: any) => playerBlock.wallet === player.wallet,
        );
        const playerWin = await this.getGamePlayerWin(
          player.wallet,
          gameDataById.id,
        );
    ...
    gameData = {
      gameInfo: {
        ...
        status: !gameDataById.contractAddress
          ? 'notStarted'
          : !playerData.gameData.isBettingComplete
            ? 'Waiting payment'
            : !gameDataById.finishedAt
              ? 'Game'
              : 'Finished',
      },
      players: players,
  ...
  async getGameStatus(gameId: number): Promise<string> {
    ...
    const blockchainData = await this.blockchainService.getGameData(
  ...
  async getRoundsInfoAlwaysWithResult(
    gameId: number,
    rounds: any,
  ...
  async getLastRoundResults(
    gameId: number,
    rounds: any[],
  ): Promise<{ wallet: string; result: number | null }[]> {
    const game = await this.getGameDataById(gameId);
    if (!game) {
      return [];
    }
  ...
  async setOrderOfThrows(
    ...
    try {
      ...
      if (allNull) {
        ...
        const nextPlayerIndex = walletsResult.findIndex(
  ...
  async sendEndTime(note: string, duration: number, gameId: number) {
    const now = Date.now();
    const endTimestamp = now + duration;

    if (!this.endTimes.has(gameId)) {
      this.endTimes.set(gameId, {});
    }

    const times = this.endTimes.get(gameId)!;
    times[note] = endTimestamp;

    const data = { endTimestamp, gameId, note };

    this.gameGateway.send(note, data, gameId);
  }

  async resendEndTimes(gameId: number) {
    const times = this.endTimes.get(gameId);
    if (!times) return;

    for (const [note, endTimestamp] of Object.entries(times)) {
      this.gameGateway.send(note, { endTimestamp, gameId, note }, gameId);
    }
  }
}
